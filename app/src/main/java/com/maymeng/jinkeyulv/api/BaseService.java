...
public interface BaseService {
    ...
    @FormUrlEncoded
    @POST("api/Login")
    Observable<LoginBean> loginUser(@Field("AccountPhone") String AccountPhone, @Field("AccountPwd") String AccountPwd, @Field("IP4") String IP4);

    //  添加案件
    @FormUrlEncoded
    @POST("api/Case")
    Observable<AddCaseBean> addCaseNet(@Field("AddAccountId") int AddAccountId, @Field("CustomerName") String CustomerName, @Field("CaseNumber") String CaseNumber, @Field("OutDangerTime") String OutDangerTime,
    ...
    @FormUrlEncoded
    @POST("api/UpdatePost")
    Observable<BaseNetBean> updateCaseNet(@Field("OrderId") int orderId, @Field("CaseNumber") String CaseNumber, @Field("OutDangerTime") String OutDangerTime,
                                          @Field("OutDangerAddress") String OutDangerAddress, @Field("CasualtiesType") String CasualtiesType, @Field("OutDangerDescription") String OutDangerDescription);
...
//    Observable<SubmitInfoBean> submitInfoNet(@Body String body);
    ...
    Observable<SubmitInfoBean> submitInfoNet(@Field("OrderId") int OrderId//派单ID
    ...
    @GET("api/SubmitOrder")
    Observable<NewDispatchBean> getNewDispatchNet(@Query("accountId") int AccountId, @Query("currentPage") int CurrentPage, @Query("pageSize") int PageSize);


    //    更改后台添加的派单信息为已读
    @FormUrlEncoded
    @POST("api/SetRead")
    Observable<BaseNetBean> setNewDispatchReadNet(@Field("orderId") int orderId);

    //    根据案件ID获取案件信息
    @GET("api/Case")
    Observable<NewDispatchCaseInfoBean> getNewDispatchCaseInfoNet(@Query("caseId") int caseId);

    //    根据用户ID获取所有关联的派单信息
    @GET("api/GetAll")
    Observable<NewDispatchBean> getAllByAccountID(@Query("accountId") int accountId, @Query("CurrentPage") int CurrentPage, @Query("PageSize") int PageSize);

    //    根据条件查询派单:比如身份证号，姓名，案件号
    @GET("api/CaseUserInfo")
    Observable<NewDispatchBean> getCaseUserInfoByOpition(@Query("accountID") int accountID, @Query("currentPage") int currentPage, @Query("pageSize") int pageSize, @Query("value") String value);


    //    文件上传
    @Multipart
    @POST("api/UploadImage")
    Observable<UploadFileBean> uploadFileNet(@Part("data") int data, @Part MultipartBody.Part file);
...
//    @FormUrlEncoded
    @POST("api/Geography")
    Observable<BaseNetBean> submitPictureInfoNet(@Body List<PictureInfoBean> list);

    //    根据用户ID获取所有未签约的派单
    @GET("api/GetSignCase")
    Observable<SignBean> getSignByAccountID(@Query("accountId") int accountId, @Query("currentPage") int currentPage, @Query("pageSize") int pageSize);

    //    提交签约文件
    @FormUrlEncoded
    @POST("api/UpdateSignCase")
    Observable<SignUploadBean> UpdateSignCaseNet(@Field("CaseId") int CaseId, @Field("EntryApprovalForm") String EntryApprovalForm, @Field("AgentContract") String AgentContract);
