            ...
            if (angle >= 90F || angle <= -90F) {
                ...
                if (firstFlag) {
                    if (!TextUtils.isEmpty(contentText)) {
                        //绘制文字，靠右并留出空隙
                        firstFlag = false;
//                        int drawRightContentStart = measuredWidth - getTextWidth(paintCenterText, contentText) - drawCenterContentStart;
                        int drawRightContentStart = getTextWidth(paintCenterText, contentText) + drawCenterContentStart;
